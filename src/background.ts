...
async function addToRecentTickets(ticketId: string) {
  try {
    ...
    if (!recentTickets.includes(ticketId)) {
      const updatedTickets = [ticketId, ...recentTickets.filter(t => t !== ticketId)].slice(0, 5);
...
chrome.runtime.onInstalled.addListener(() => {
  ...
  getSettingsWithCache().catch(err => console.error('Error initializing settings cache:', err));
}); 