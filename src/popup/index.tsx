          ...
          return (
            ...
            >
              {customIconPath ? (
                ...
                <span className="w-6 h-6 mb-0.5 flex items-center justify-center">{icons[env.icon]}</span>
              )}
...
const JiraUrlWizard = () => {
  ...
  const handleCopyEnvironmentLinks = useCallback(() => {
    const urls = settings?.urls && typeof settings.urls === 'object'
                  ? settings.urls
                  : DEFAULT_SETTINGS.urls;
    const templateText = `# 🌐 Frontend Environments
${urls.bo ? `- **Back Office Tool** → [${urls.bo}](${urls.bo})` : ''}
${urls.mobile ? `- **Mobile Version** → [${urls.mobile}](${urls.mobile})` : ''}
${urls.desktop ? `- **Desktop Version** → [${urls.desktop}](${urls.desktop})` : ''}
...
${urls.drupal7 ? `
## **Drupal 7**
- **Base CMS** → [${urls.drupal7}](${urls.drupal7})
- **Desktop View** → [${urls.drupal7}/fahrradversicherung?deviceoutput=desktop](${urls.drupal7}/fahrradversicherung?deviceoutput=desktop)
- **Mobile View** → [${urls.drupal7}/fahrradversicherung?deviceoutput=mobile](${urls.drupal7}/fahrradversicherung?deviceoutput=mobile)` : ''}
${urls.drupal9 ? `
## **Drupal 9**
- **Desktop View** → [${urls.drupal9}/fahrradversicherung?deviceoutput=desktop](${urls.drupal9}/fahrradversicherung?deviceoutput=desktop)
- **Mobile View** → [${urls.drupal9}/fahrradversicherung?deviceoutput=mobile](${urls.drupal9}/fahrradversicherung?deviceoutput=mobile)` : ''}
`.trim().replace(/^\s*[\r\n]/gm, '');

    copyToClipboard(templateText, "Environment links copied!");
  }, [copyToClipboard, settings?.urls]);
